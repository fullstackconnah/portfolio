...
function ProjectCard({ title, description, tech = [], id, featured = false }) {
  ...
  const truncatedDescription = truncateText(description, featured ? 250 : 150);
  return (
    <Link
      to={`/projects/${id}`}
      className="block"
    >
      <div className={`relative group bg-black flex flex-col justify-between h-full border border-[#39FF14] text-[#39FF14] font-mono rounded-lg ${featured ? 'p-6' : 'p-5'} shadow-[0_0_4px_#39FF14]/40 transition duration-300 hover:shadow-[0_0_16px_#39FF14] hover:scale-[1.015] hover:animate-terminalFlicker overflow-hidden cursor-pointer card-3d-hover`}>
